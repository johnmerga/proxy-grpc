...
export class GrpcClientService implements OnModuleInit {
  ...
  private async initializeClient(): Promise<void> {
    try {
      ...
      if (!protoDescriptor.proxySng || !protoDescriptor.proxySng) {
        throw new Error(
      ...
      if (typeof sngService !== 'function') {
        throw new Error('Sng service is not a constructor');
      }

      // Create client instance
      this.client = new sngService(
        'localhost:9001', // Update with your gRPC server address
